...
#else
#define _P_clogger_lock_(level) ({ 0; })
#define _P_clogger_unlock_(level) ({ 0; })
#define _P_pthread_mutex_lock_(mutex) ({ 0; })
#define _P_pthread_mutex_unlock_(mutex) ({ 0; })
#endif

#define _P_clogger_worker_(ustream, level, ...)                               \
  do                                                                          \
    {                                                                         \
      if (level == NONE)                                                      \
        break;                                                                \
      _P_clogger_lock_ (level);                                               \
      void *ustream = static$ (clogger_level_config)[level].mbr$ (stream);    \
      if (ustream == &static$ (clogger_level_config)[NONE])                   \
        {                                                                     \
          _P_clogger_unlock_ (level);                                         \
          break;                                                              \
        }                                                                     \
      if (!ustream)                                                           \
        ustream = static$ (clogger_level_config)[level].mbr$ (stream)         \
            = stderr;                                                         \
      if (level == ASSERT)                                                    \
        {                                                                     \
        ...
        _P_clogger_do_log_ (ustream, level __VA_OPT__ (, ) __VA_ARGS__);      \
      _P_clogger_unlock_ (level);                                             \
...
#define clogger(level, ...)                                                   \
  ({                                                                          \
    _P_clogger_worker_ (uniq$ (ustream), level __VA_OPT__ (, ) __VA_ARGS__);  \
    (void)0;                                                                  \
  })

int $fn (clogger, update_level_config) (enum clogger_level level);

#define _P_clgredir_(ulock, level, astream)                                   \
  do                                                                          \
    {                                                                         \
      if (level == NONE)                                                      \
        break;                                                                \
      ...
      static$ (clogger_level_config)[level].mbr$ (stream) = astream;          \
      $fn (clogger, update_level_config) (level);                             \
      _P_pthread_mutex_unlock_ (ulock);                                       \
    }                                                                         \
  while (0)
#define clgredir(level, stream) _P_clgredir_ (uniq$ (ulock), level, stream)
