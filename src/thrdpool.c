...
$fx (thrdpool, cleanup)
{
  ...
  free (this->mbr$ (tid));
  for (xapr *node;
       node = xlist$ (shift) (&this->mbr$ (exec_deque).mbr$ (head),
                              &this->mbr$ (exec_deque).mbr$ (tail));)
    {
  ...
  xlsfree$ (&this->mbr$ (cancel_deque).mbr$ (head),
            &this->mbr$ (cancel_deque).mbr$ (tail));
...
$fx (thrdpool, deinit)
{
  $fn (blocking_deque, push) (&this->mbr$ (cancel_deque), &this->mbr$ (tail));
  return null;
}
...
#define gentpexec(method, op)                                                 \
  ...
  {                                                                           \
    ...
    $fn (blocking_deque, op) (&this->mbr$ (exec_deque), &task->list_entry);   \
    return 0;                                                                 \
  }
gentpexec (exec, push);
...
EXEC_DEQUE_INIT_FAILURE:
  ...
  pthread_join (this->mbr$ (cancel_thread), null);
